# HMDA Book 1 V4 — Publication logic ledger

Final V4 publication state: **PASS**. Input mode: **Final V4 machine gate**.
...
- Encoded case checks: 30/30 PASS.
- Publication issues: 0.
- PDF map geometry: PASS.
...
Run `python scripts/validate_book1_v4_logic.py --master dist/book1_en_master_owner_review_v4.yml --runtime dist/hmda_spatial_runtime_v3.json`. Use the actual final runtime path if different. The matching JSON report records solution counts, counterexamples, domains, constraints and input hashes.

```json
{
  "master": {
    "path": "dist\\book1_en_master_owner_review_v4.yml",
    "sha256": "0a61b726e6335cc731f114749db632b6b0a50f2a8da76ef763a8961e066feacc"
  },
  "runtime": {
    "path": "dist\\hmda_spatial_runtime_v4.json",
    "sha256": "06bb284162ab62e993c8eee9237c58b67b6b545a425d2b04085725a81d50afc8"
  },
  "pdf": {
    "path": "dist\\HMDA_Book 1_EN_OwnerReview_v4.pdf",
    "sha256": "1b2b908763e76b42321035fb1faf91056bd2a8a1301ea0a2f84944c91287e1cb"
  },
  "page_index": {
    "path": "dist\\HMDA_Book 1_EN_OwnerReview_v4_page_index.json",
    "sha256": "309176917a571fbd13b2e8ac15bec850439793acb20308bcc1d389f56d6c56cb"
  }
}
```
...
- **Narrative setup:** The black envelope from the opening is now on the Academy’s evidence mat, with the unassigned badge beside it. Its receipt says INTERNAL BADGE STOCK and EXTERNAL DELIVERY: NONE. Four visiting helpers—Ari, Bea, Cole and Dani—were recorded when it emerged. Use that snapshot to find who was at Delivery.
- **Objective:** IDENTIFY who was at Delivery when the black envelope emerged.
- **Puzzle type:** guided.
- **Happy Makers:** MIMI, LULI, ALIO.
...
- **Room Zero / meta signal:** {"code": "INTAKE-01", "label": "BADGE REVERSE", "reaction": "You found the intake witness. I logged your verdict; the desk has released the next file.", "reader_move": "Add INTAKE-01 to the Case Wall on your Field Detective ID.", "speaker": "mimi", "status": "FIRST RECORD LOGGED"}
- **Later callback / required reuse:** First five-case recap; Cases 09/28 recurring-signal audit. Opening ID is reused in30.
- **Final QA status:** Logic **PASS** for the bound inputs. Rendered visual evidence, numeric marker clarity, clues-before-puzzle placement and page references require the separate V4 PDF and visual audits.

## Case 02 — THE TROPHY THAT NEVER REACHED THE SHELF

- **Narrative setup:** A reopened sports-showcase file records a trophy leaving its shelf. Max was the trophy keeper. Rebuild the recorded positions to identify the one person who shared Max’s room; that contact is the next useful person to ask about the trophy.
- **Objective:** FIND Max’s only room companion and record their coordinate.
- **Puzzle type:** spatial.
...
- **Visual asset (source_page_asset):** `dist/map_factory_owner_review_v4/HMDA_02_puzzle_original_shigai_relabelled.png`.
- **Visual asset (puzzle_asset):** `dist/map_factory_owner_review_v4/HMDA_02_puzzle_original_shigai_relabelled.png`.
- **Visual asset (solution_asset):** `dist/map_factory_owner_review_v4/HMDA_02_solution_original_shigai_relabelled.png`.
...
5. Ivy was standing on a desk chair.
...
- **Room Zero / meta signal:** {"code": "INTAKE-02", "label": "TROPHY EVIDENCE TAG", "reaction": "The trophy has a shelf problem. Its tag has a familiar mark.", "reader_move": "Keep the completed map. Add TROPHY TAG to your Case Wall.", "speaker": "alio", "status": "SECOND MARK"}
...
- **Narrative setup:** The trophy file includes two Academy evidence snapshots. Compare the parcel, its tag and the muddy print. The printed relevance rule explains which kinds of change matter. Background decoration must not create an extra answer.
- **Objective:** CIRCLE the three meaningful changes under the printed relevance rule.
- **Puzzle type:** visual.
...
- **Room Zero / meta signal:** {"code": "INTAKE-03", "label": "RECORD 0471", "reaction": "The front changes the evidence trail. The back gives us a match.", "reader_move": "Keep the three meaningful changes and log TAG 0471.", "speaker": "nini", "status": "MATCH ON THE REVERSE"}
...
- **Narrative setup:** The maker-fair kitchen mixed up a test-batch cupcake box and an ordinary collection. Pixel kept the collection list. The saved floor record identifies the only person in Pixel’s room when the mix-up was logged.
- **Objective:** LOCATE Pixel’s one room contact. Give name and square.
- **Puzzle type:** spatial.
...
- **Solvability evidence:** Exhaustive CSP found 1 complete assignment(s), canonical match=True, search nodes=1. No answer position was an input constraint.
- **Timing assumption:** All observations are simultaneous at the stated incident/record time; no witness moves between clues. The named companion only answers the physical-incident question when that incident’s handover/access link is explicit.
- **Visual asset (source_page_asset):** `dist/map_factory_owner_review_v4/HMDA_04_puzzle_original_shigai_relabelled.png`.
- **Visual asset (puzzle_asset):** `dist/map_factory_owner_review_v4/HMDA_04_puzzle_original_shigai_relabelled.png`.
- **Visual asset (solution_asset):** `dist/map_factory_owner_review_v4/HMDA_04_solution_original_shigai_relabelled.png`.
...
- **Room Zero / meta signal:** {"code": "INTAKE-04", "label": "CUPCAKE INTAKE LABEL", "reaction": "Box enquiry filed. Its intake sticker can join our collection of questions.", "reader_move": "Log CUPCAKE LABEL after recording the map.", "speaker": "mimi", "status": "FOURTH SIGNAL"}
- **Later callback / required reuse:** Case 26 revisits this map; the empty ROOM initial joins CHECK THE OLD MAP. Cases 27–30 depend on that instruction.
...
- **Narrative setup:** Dilo has linked the field evidence locker to the intake desk. Its active four-symbol sequence is saved automatically in the session record. An old sealed file is inside, and the keypad gives four clues to its order.
- **Objective:** DECODE the one four-symbol order fitting every clue.
- **Puzzle type:** code.
...
- **Room Zero / meta signal:** {"code": "INTAKE-05", "label": "LOCKER ENVELOPE", "reaction": "The code worked. The envelope is old. Two different kinds of interesting.", "reader_move": "Keep the solved sequence. Add LOCKER ENVELOPE as the fifth signal.", "speaker": "dilo", "status": "FIVE REAL SIGNALS"}
...
- **Narrative setup:** The museum asks the Academy to reopen a label mix-up: a Roman spoon was described as a CEREMONIAL DRAGON TOOTH. Uma checked the display labels. Find the one person sharing Uma’s recorded room before the team asks the next question.
- **Objective:** FIND the witness sharing Uma’s room. Record their square.
- **Puzzle type:** spatial.
- **Happy Makers:** LULI, NINI.
...
- **Solvability evidence:** Exhaustive CSP found 1 complete assignment(s), canonical match=True, search nodes=1. No answer position was an input constraint.
- **Timing assumption:** All observations are simultaneous at the stated incident/record time; no witness moves between clues. The named companion only answers the physical-incident question when that incident’s handover/access link is explicit.
- **Visual asset (source_page_asset):** `dist/map_factory_owner_review_v4/HMDA_06_puzzle_original_shigai_relabelled.png`.
- **Visual asset (puzzle_asset):** `dist/map_factory_owner_review_v4/HMDA_06_puzzle_original_shigai_relabelled.png`.
- **Visual asset (solution_asset):** `dist/map_factory_owner_review_v4/HMDA_06_solution_original_shigai_relabelled.png`.
...
- **Room Zero / meta signal:** {"code": "QUEUE-06", "label": "MUSEUM LABEL COPY", "reaction": "An older file has joined our question. At least the spoon has its name back.", "reader_move": "Mark the matching shape; leave its meaning open.", "speaker": "nini", "status": "ARCHIVE ECHO"}
- **Later callback / required reuse:** Case 26 revisits this map; the empty ROOM initial joins CHECK THE OLD MAP. Cases 27–30 depend on that instruction.
...
- **Narrative setup:** A film-club record shows the giant moon costume missing before rehearsal. Gray handled the inventory. The backstage snapshot identifies the only person sharing Gray’s area. Alio has put on the spare demonstration costume while the team studies the file.
- **Objective:** REBUILD the positions. Name Gray’s one companion and their square.
- **Puzzle type:** spatial.
- **Happy Makers:** ALIO, MIMI, NINI.
...
- **Timing assumption:** All observations are simultaneous at the stated incident/record time; no witness moves between clues. The named companion only answers the physical-incident question when that incident’s handover/access link is explicit.
- **Visual asset (source_page_asset):** `dist/map_factory_owner_review_v4/HMDA_07_puzzle_original_shigai_relabelled.png`.
- **Visual asset (puzzle_asset):** `dist/map_factory_owner_review_v4/HMDA_07_puzzle_original_shigai_relabelled.png`.
- **Visual asset (solution_asset):** `dist/map_factory_owner_review_v4/HMDA_07_solution_original_shigai_relabelled.png`.
...
- **Room Zero / meta signal:** {"code": "QUEUE-07", "label": "COSTUME PAPER RECORD", "reaction": "The moon did not write its own paperwork. Keep the inventory copy.", "reader_move": "Keep the paper record with the completed map.", "speaker": "mimi", "status": "NEW CONTEXT"}
- **Later callback / required reuse:** Case 26 revisits this map; the empty ROOM initial joins CHECK THE OLD MAP. Cases 27–30 depend on that instruction.
...
- **Narrative setup:** The team needs the prop room at 16:20. Alio has drawn three routes. Wet paint, a locked staff door and a wall block some options. They must arrive now; waiting for a closure to end is not allowed.
- **Objective:** CHOOSE the only permitted route to the prop room.
- **Puzzle type:** route.
...
- **Room Zero / meta signal:** {"code": "CHECK-08", "label": "ROUTE CHECK", "reaction": "I have retired one shortcut. The wall seemed relieved.", "reader_move": "Keep the legal route and the rule that proved it.", "speaker": "alio", "status": "A USEFUL REJECTION"}
...
- **Narrative setup:** Several case records carry a plain 0. A poster circle and stage-light dial also look round. Decide which samples really match. Bibi remembers the Academy form, but a memory is not a complete explanation.
- **Objective:** MATCH the true Academy marks and exclude the lookalikes.
...
- **Room Zero / meta signal:** {"code": "ARCHIVE-09", "label": "BIBI’S ARCHIVE MEMORY", "reaction": "I used forms with that mark. What its zero meant is the part I need to check.", "reader_move": "Write FACT: BIBI RECOGNIZES THE OLD FORM.", "speaker": "bibi", "status": "PATTERN CONFIRMED"}
...
- **Narrative setup:** Two science-fair teams claimed the same silver rover. Dash kept the prototype access record. The saved positions identify who shared Dash’s area, giving the team a contact to question before deciding anything about ownership.
- **Objective:** IDENTIFY the contact sharing Dash’s prototype area and their square.
- **Puzzle type:** spatial.
- **Happy Makers:** DILO, LULI.
...
- **Timing assumption:** All observations are simultaneous at the stated incident/record time; no witness moves between clues. The named companion only answers the physical-incident question when that incident’s handover/access link is explicit.
- **Visual asset (source_page_asset):** `dist/map_factory_owner_review_v4/HMDA_10_puzzle_original_shigai_relabelled.png`.
- **Visual asset (puzzle_asset):** `dist/map_factory_owner_review_v4/HMDA_10_puzzle_original_shigai_relabelled.png`.
- **Visual asset (solution_asset):** `dist/map_factory_owner_review_v4/HMDA_10_solution_original_shigai_relabelled.png`.
...
- **Room Zero / meta signal:** {"code": "QUEUE-10", "label": "ROVER DIAGNOSTIC", "reaction": "Cable opinions postponed. Our detective has given us a contact to check.", "reader_move": "Keep all final witness positions and the diagnostic record.", "speaker": "dilo", "status": "RECORD SAVED"}
- **Later callback / required reuse:** Case 26 revisits this map; the empty ROOM initial joins CHECK THE OLD MAP. Cases 27–30 depend on that instruction.
...
- **Narrative setup:** Mimi empties the evidence bag from the science fair. Three case items, two pens, an oat bar and her missing phone land on the table. Compare only the timestamped records A–D against the printed 15:20–16:05 case window.
- **Objective:** FIND which timestamped record A–D falls outside 15:20–16:05.
...
- **Room Zero / meta signal:** {"code": "ARCHIVE-11", "label": "08:07 PHOTO", "reaction": "That photo cannot be a fair-time record. It can help our other question.", "reader_move": "Keep the 08:07 photograph as archive context.", "speaker": "nini", "status": "WRONG CASE, USEFUL DETAIL"}
...
- **Narrative setup:** A theatre parrot repeated a four-word phrase nobody remembered teaching it. Winter was the handler named in the rehearsal record. The positions show who shared Winter’s area; the parrot’s snack requests are not extra clues.
- **Objective:** MATCH Winter to the one other person in the room. Give their coordinate.
- **Puzzle type:** spatial.
...
- **Solvability evidence:** Exhaustive CSP found 1 complete assignment(s), canonical match=True, search nodes=9. No answer position was an input constraint.
- **Timing assumption:** All observations are simultaneous at the stated incident/record time; no witness moves between clues. The named companion only answers the physical-incident question when that incident’s handover/access link is explicit.
- **Visual asset (source_page_asset):** `dist/map_factory_owner_review_v4/HMDA_12_puzzle_original_shigai_relabelled.png`.
- **Visual asset (puzzle_asset):** `dist/map_factory_owner_review_v4/HMDA_12_puzzle_original_shigai_relabelled.png`.
- **Visual asset (solution_asset):** `dist/map_factory_owner_review_v4/HMDA_12_solution_original_shigai_relabelled.png`.
...
- **Room Zero / meta signal:** {"code": "QUEUE-12", "label": "PARROT CUE-CARD COPY", "reaction": "The witness has answered further questions with cracker. We still have the record.", "reader_move": "Log the cue-card copy after saving the positions.", "speaker": "dilo", "status": "PAPER TRAIL"}
- **Later callback / required reuse:** Case 26 revisits this map; the empty ROOM initial joins CHECK THE OLD MAP. Cases 27–30 depend on that instruction.
...
- **Narrative setup:** An animal-centre camera recorded a blurry image at 4:17 and was later found behind supplies. Remy kept its maintenance record. The saved positions identify Remy’s one room companion. The fox is not on the witness list.
- **Objective:** FIND the contact sharing Remy’s room and their square.
- **Puzzle type:** spatial.
- **Happy Makers:** LULI, ALIO.
...
- **Timing assumption:** All observations are simultaneous at the stated incident/record time; no witness moves between clues. The named companion only answers the physical-incident question when that incident’s handover/access link is explicit.
- **Visual asset (source_page_asset):** `dist/map_factory_owner_review_v4/HMDA_13_puzzle_original_shigai_relabelled.png`.
- **Visual asset (puzzle_asset):** `dist/map_factory_owner_review_v4/HMDA_13_puzzle_original_shigai_relabelled.png`.
- **Visual asset (solution_asset):** `dist/map_factory_owner_review_v4/HMDA_13_solution_original_shigai_relabelled.png`.
...
- **Room Zero / meta signal:** {"code": "QUEUE-13", "label": "CAMERA MAINTENANCE CARD", "reaction": "Your notes give us something better than a theory: a trail we can check.", "reader_move": "Keep the map and maintenance record together.", "speaker": "luli", "status": "OUR DETECTIVE’S RECORD"}
- **Later callback / required reuse:** Case 26 revisits this map; the empty ROOM initial joins CHECK THE OLD MAP. Cases 27–30 depend on that instruction.
...
- **Narrative setup:** Four actors remember a backstage mix-up. The route map and clock let you check their accounts. One stated journey takes longer than the witness allows. A mistaken memory does not make somebody dishonest.
- **Objective:** IDENTIFY the statement whose journey cannot fit its times.
...
- **Room Zero / meta signal:** {"code": "CHECK-14", "label": "WITNESS TIMING RECORD", "reaction": "We corrected the journey without turning a muddled memory into a crime.", "reader_move": "Keep proved travel time separate from theories about Room Zero.", "speaker": "nini", "status": "MEMORY TESTED"}
...
- **Narrative setup:** An Academy camp recorded a backpack mix-up before departure. Milo’s bag was involved. Use the position record to identify the person sharing Milo’s room, so the team knows which contact to ask next.
- **Objective:** NAME the witness in Milo’s room and mark their location.
- **Puzzle type:** spatial.
...
- **Timing assumption:** All observations are simultaneous at the stated incident/record time; no witness moves between clues. The named companion only answers the physical-incident question when that incident’s handover/access link is explicit.
- **Visual asset (source_page_asset):** `dist/map_factory_owner_review_v4/HMDA_15_puzzle_original_shigai_relabelled.png`.
- **Visual asset (puzzle_asset):** `dist/map_factory_owner_review_v4/HMDA_15_puzzle_original_shigai_relabelled.png`.
- **Visual asset (solution_asset):** `dist/map_factory_owner_review_v4/HMDA_15_solution_original_shigai_relabelled.png`.
...
- **Room Zero / meta signal:** {"code": "QUEUE-15", "label": "REPLACEMENT LUGGAGE RECORD", "reaction": "A readable tag helped the bag. Its old form gives us something to compare.", "reader_move": "Keep the form and all seven witness positions.", "speaker": "mimi", "status": "FAMILIAR FORM"}
- **Later callback / required reuse:** Case 26 revisits this map; the empty ROOM initial joins CHECK THE OLD MAP. Cases 27–30 depend on that instruction.
...
- **Narrative setup:** Bibi lays down an old photograph of five children at the Academy. She points herself out beneath an enormous hat. These are her childhood squadmates, not the present-day field detectives. Use the printed evidence to establish where and when the photograph was taken.
- **Objective:** WORK OUT the photograph’s building and year.
...
- **Room Zero / meta signal:** {"code": "ARCHIVE-16", "label": "PHOTO REVERSE", "reaction": "My handwriting. My ridiculous hat. One of those is a useful clue.", "reader_move": "Copy RULE 0 FIRST to the Case Wall. Leave its meaning open.", "speaker": "bibi", "status": "RULE 0 FIRST"}
...
- **Narrative setup:** A railway open-day file records a special ticket changing hands. Reed was the ticket contact. Identify the only person sharing Reed’s recorded area. The clocks need a separate check in the next file.
- **Objective:** TRACE the ticket record to Reed’s one room contact. Give their coordinate.
- **Puzzle type:** spatial.
...
- **Timing assumption:** All observations are simultaneous at the stated incident/record time; no witness moves between clues. The named companion only answers the physical-incident question when that incident’s handover/access link is explicit.
- **Visual asset (source_page_asset):** `dist/map_factory_owner_review_v4/HMDA_17_puzzle_original_shigai_relabelled.png`.
- **Visual asset (puzzle_asset):** `dist/map_factory_owner_review_v4/HMDA_17_puzzle_original_shigai_relabelled.png`.
- **Visual asset (solution_asset):** `dist/map_factory_owner_review_v4/HMDA_17_solution_original_shigai_relabelled.png`.
...
- **Room Zero / meta signal:** {"code": "QUEUE-17", "label": "TICKET RECORD SLEEVE", "reaction": "I kept the ticket with its sleeve. Even my helmet could not lose that distinction.", "reader_move": "Keep the ticket record for the timing file next.", "speaker": "alio", "status": "ANOTHER PAPER MATCH"}
- **Later callback / required reuse:** Case 26 revisits this map; the empty ROOM initial joins CHECK THE OLD MAP. Cases 27–30 depend on that instruction.
...
- **Narrative setup:** The ticket records use three clocks. The café clock is seven minutes slow, Dilo’s watch is two minutes fast, and the station camera shows real time. Convert all four records before ordering them.
- **Objective:** ORDER the four events by their corrected real times.
...
- **Room Zero / meta signal:** {"code": "CHECK-18", "label": "0:07", "reaction": "One clock corrected. One exciting zero theory cancelled.", "reader_move": "Mark FALSE LEAD: TIME FORMAT. Keep the correct order.", "speaker": "dilo", "status": "FALSE LEAD"}
...
- **Narrative setup:** A media-library record says a rare book was checked out, although it stayed on the shelf. Sasha was at the scanner enquiry. Find Sasha’s one room companion. Proximity alone does not prove who caused the error.
- **Objective:** NAME the only person sharing Sasha’s area. Record their square.
- **Puzzle type:** spatial.
- **Happy Makers:** DILO, LULI.
...
- **Solvability evidence:** Exhaustive CSP found 1 complete assignment(s), canonical match=True, search nodes=16. No answer position was an input constraint.
- **Timing assumption:** All observations are simultaneous at the stated incident/record time; no witness moves between clues. The named companion only answers the physical-incident question when that incident’s handover/access link is explicit.
- **Visual asset (source_page_asset):** `dist/map_factory_owner_review_v4/HMDA_19_puzzle_original_shigai_relabelled.png`.
- **Visual asset (puzzle_asset):** `dist/map_factory_owner_review_v4/HMDA_19_puzzle_original_shigai_relabelled.png`.
- **Visual asset (solution_asset):** `dist/map_factory_owner_review_v4/HMDA_19_solution_original_shigai_relabelled.png`.
...
- **Room Zero / meta signal:** {"code": "QUEUE-19", "label": "SCANNER RECORD", "reaction": "The mark sits in a record field. Perhaps the place around it matters too.", "reader_move": "Log the blank-field detail without deciding its meaning.", "speaker": "nini", "status": "A FIELD LEFT BLANK"}
- **Later callback / required reuse:** Case 26 revisits this map; the empty ROOM initial joins CHECK THE OLD MAP. Cases 27–30 depend on that instruction.
...
- **Narrative setup:** A design studio recorded painted panels in the wrong order. Demi handled the panel records. Nobody is being accused. Identify Demi’s only room companion so the team can continue the enquiry.
- **Objective:** RECORD the name and square of Demi’s studio-room companion.
- **Puzzle type:** spatial.
- **Happy Makers:** MIMI, NINI.
...
- **Timing assumption:** All observations are simultaneous at the stated incident/record time; no witness moves between clues. The named companion only answers the physical-incident question when that incident’s handover/access link is explicit.
- **Visual asset (source_page_asset):** `dist/map_factory_owner_review_v4/HMDA_20_puzzle_original_shigai_relabelled.png`.
- **Visual asset (puzzle_asset):** `dist/map_factory_owner_review_v4/HMDA_20_puzzle_original_shigai_relabelled.png`.
- **Visual asset (solution_asset):** `dist/map_factory_owner_review_v4/HMDA_20_solution_original_shigai_relabelled.png`.
...
- **Room Zero / meta signal:** {"code": "QUEUE-20", "label": "PANEL DRYING CARD", "reaction": "We know where the witnesses were. Our detective’s map is staying in the file.", "reader_move": "Preserve all final positions. Do not erase a solved map.", "speaker": "mimi", "status": "EVIDENCE KEPT"}
- **Later callback / required reuse:** Case 26 revisits this map; the empty ROOM initial joins CHECK THE OLD MAP. Cases 27–30 depend on that instruction.
...
- **Narrative setup:** The intake tray releases a brittle note. Dilo pulls the folded edge too quickly; four pieces settle on the mat. Their tear edges and handwriting are preserved. Rebuild the note from the visible joins and the sentence.
- **Objective:** REBUILD the four-piece note and write its instruction.
...
- **Room Zero / meta signal:** {"code": "MESSAGE-21", "label": "THE REBUILT NOTE", "reaction": "It asks us to notice something that is not there. We can test that question.", "reader_move": "Copy the exact instruction after rebuilding all four pieces.", "speaker": "nini", "status": "A NEW KIND OF CLUE"}
- **Later callback / required reuse:** The empty-space instruction is required to motivate Case 26.
- **Final QA status:** Logic **PASS** for the bound inputs. Rendered visual evidence, numeric marker clarity, clues-before-puzzle placement and page references require the separate V4 PDF and visual audits.

## Case 22 — THE MUSIC ROOM MIX-UP

- **Narrative setup:** Three identical instrument cases were mixed up at the music school. Zara kept the inventory enquiry. Identify the one person sharing Zara’s recorded room before following the instrument trail.
- **Objective:** LINK Zara to the one witness in the same room. Record that witness’s square.
- **Puzzle type:** spatial.
- **Happy Makers:** LULI, ALIO.
...
- **Timing assumption:** All observations are simultaneous at the stated incident/record time; no witness moves between clues. The named companion only answers the physical-incident question when that incident’s handover/access link is explicit.
- **Visual asset (source_page_asset):** `dist/map_factory_owner_review_v4/HMDA_22_puzzle_original_shigai_relabelled.png`.
- **Visual asset (puzzle_asset):** `dist/map_factory_owner_review_v4/HMDA_22_puzzle_original_shigai_relabelled.png`.
- **Visual asset (solution_asset):** `dist/map_factory_owner_review_v4/HMDA_22_solution_original_shigai_relabelled.png`.
...
- **Room Zero / meta signal:** {"code": "QUEUE-22", "label": "INSTRUMENT INVENTORY", "reaction": "Do not trim the record down to the answer. The other placements are evidence too.", "reader_move": "Keep the full placement, including every witness number.", "speaker": "luli", "status": "KEEP THE WHOLE MAP"}
- **Later callback / required reuse:** Case 26 revisits this map; the empty ROOM initial joins CHECK THE OLD MAP. Cases 27–30 depend on that instruction.
...
- **Narrative setup:** A park file records two confused wristbands: one for climbing, one for arcade access. Jude kept the return record. Find the person sharing Jude’s area. Being nearby is a lead, not proof of blame.
- **Objective:** FIND the person sharing Jude’s area and record their square.
- **Puzzle type:** spatial.
- **Happy Makers:** MIMI, DILO.
...
- **Timing assumption:** All observations are simultaneous at the stated incident/record time; no witness moves between clues. The named companion only answers the physical-incident question when that incident’s handover/access link is explicit.
- **Visual asset (source_page_asset):** `dist/map_factory_owner_review_v4/HMDA_23_puzzle_original_shigai_relabelled.png`.
- **Visual asset (puzzle_asset):** `dist/map_factory_owner_review_v4/HMDA_23_puzzle_original_shigai_relabelled.png`.
- **Visual asset (solution_asset):** `dist/map_factory_owner_review_v4/HMDA_23_solution_original_shigai_relabelled.png`.
...
5. Mae shared a room with a person who was on a bench. That person was either Ozzy or Pia.
...
- **Room Zero / meta signal:** {"code": "QUEUE-23", "label": "WRISTBAND RETURN RECORD", "reaction": "The scoreboard can wait. Our detective has a better record to finish.", "reader_move": "Keep the return record and the full map.", "speaker": "dilo", "status": "ANOTHER COMPLETED FILE"}
- **Later callback / required reuse:** Case 26 revisits this map; the empty ROOM initial joins CHECK THE OLD MAP. Cases 27–30 depend on that instruction.
...
- **Narrative setup:** One uninterrupted walk crossed the controlled mud track. No fresh mud was added between prints. The tread arrows point one way; the amount of transferred mud may tell you something different. Use the stated transfer rule.
- **Objective:** TRACK the true direction of travel from the fading mud.
...
- **Room Zero / meta signal:** {"code": "CHECK-24", "label": "MUD, NOT THE ARROW", "reaction": "My theory waited. The mud answered. I could get used to this.", "reader_move": "Keep the direction evidence. Leave the decorative arrow out of the theory.", "speaker": "alio", "status": "OBSERVATION WINS"}
...
- **Narrative setup:** At the Academy guest house, helpful people passed a parcel along until its destination became unclear. Nori was the last named contact on the enquiry. Identify the one person sharing Nori’s recorded area.
- **Objective:** REBUILD the parcel snapshot. Name Nori’s room companion and their square.
- **Puzzle type:** spatial.
...
- **Timing assumption:** All observations are simultaneous at the stated incident/record time; no witness moves between clues. The named companion only answers the physical-incident question when that incident’s handover/access link is explicit.
- **Visual asset (source_page_asset):** `dist/map_factory_owner_review_v4/HMDA_25_puzzle_original_shigai_relabelled.png`.
- **Visual asset (puzzle_asset):** `dist/map_factory_owner_review_v4/HMDA_25_puzzle_original_shigai_relabelled.png`.
- **Visual asset (solution_asset):** `dist/map_factory_owner_review_v4/HMDA_25_solution_original_shigai_relabelled.png`.
...
- **Room Zero / meta signal:** {"code": "QUEUE-25", "label": "PARCEL ROUTING SLIP", "reaction": "All fourteen records are here. Our detective kept more than the answers.", "reader_move": "Keep the complete map. Case 26 will tell you what to compare.", "speaker": "mimi", "status": "FOURTEEN MAPS COMPLETE"}
- **Later callback / required reuse:** Case 26 revisits this map; the empty ROOM initial joins CHECK THE OLD MAP. Cases 27–30 depend on that instruction.
...
- **Narrative setup:** Nini puts a completed map beside the reconstructed note. Could the important part be a room with nobody in it? Your recorded positions let you test that idea across fourteen cases. Read only spaces marked ROOM.
- **Objective:** FIND each unique empty ROOM and read its initial in case order.
- **Puzzle type:** room-zero-checkpoint.
...
- **Room Zero / meta signal:** {"code": "MESSAGE-26", "label": "EMPTY-ROOM INITIALS", "reaction": "Nini asked the question. You checked fourteen maps. The message belongs to that work.", "reader_move": "Write the earned instruction and follow it to Case 27.", "speaker": "luli", "status": "MESSAGE EARNED"}
...
- **Narrative setup:** The earned message sends Bibi to the old plans drawer. Her Academy plan and the current plan share three permanent landmarks. Align them at the stated scale and find what the renovation left behind the Archive wall.
- **Objective:** ALIGN the plans and locate the sealed room.
...
- **Room Zero / meta signal:** {"code": "ARCHIVE-27", "label": "THE SEALED SPACE", "reaction": "The landmarks agree. I have brought one finger, as authorized.", "reader_move": "Mark the matching rectangle on both plans.", "speaker": "alio", "status": "PLANS ALIGNED"}
- **Later callback / required reuse:** The sealed-space observation becomes FACT in28; RULE FIRST, ROOM SECOND orders the finale.
- **Final QA status:** Logic **PASS** for the bound inputs. Rendered visual evidence, numeric marker clarity, clues-before-puzzle placement and page references require the separate V4 PDF and visual audits.

## Case 28 — RULE ZERO

- **Narrative setup:** The old plan says RULE FIRST, ROOM SECOND. Sort what the team has earned into fact, theory and unsupported assumption. Bibi remembers the full rule when the evidence brings the old training method back.
- **Objective:** SORT claims into FACT, THEORY and UNSUPPORTED ASSUMPTION.
...
- **Room Zero / meta signal:** {"code": "RULE-28", "label": "RULE ZERO", "reaction": "That was how we began an investigation. You have just practised it.", "reader_move": "Keep the missing word with the final evidence.", "speaker": "bibi", "status": "METHOD RESTORED"}
- **Later callback / required reuse:** ASSUMPTIONS is required by Case 30 RULE stage.
- **Final QA status:** Logic **PASS** for the bound inputs. Rendered visual evidence, numeric marker clarity, clues-before-puzzle placement and page references require the separate V4 PDF and visual audits.

## Case 29 — THE MAP BENEATH THE MAP

- **Narrative setup:** The old plan includes a service-level position record guarding the last access coordinate. Seth is its named contact. Solve the largest map, identify Seth’s only room companion and record that person’s coordinate.
- **Objective:** SOLVE the final map. Give Seth’s room companion and the access coordinate.
...
- **Timing assumption:** All observations are simultaneous at the stated incident/record time; no witness moves between clues. The named companion only answers the physical-incident question when that incident’s handover/access link is explicit.
- **Visual asset (source_page_asset):** `dist/map_factory_owner_review_v4/HMDA_29_puzzle_original_shigai_relabelled.png`.
- **Visual asset (puzzle_asset):** `dist/map_factory_owner_review_v4/HMDA_29_puzzle_original_shigai_relabelled.png`.
- **Visual asset (solution_asset):** `dist/map_factory_owner_review_v4/HMDA_29_solution_original_shigai_relabelled.png`.
...
- **Room Zero / meta signal:** {"code": "ACCESS-29", "label": "FINAL ACCESS RECORD", "reaction": "Verdict checked. Victory noise is now available at a respectful volume.", "reader_move": "Bring the Case 05 code, Case 28 word and Field Detective ID.", "speaker": "dilo", "status": "D3 VERIFIED"}
...
- **Narrative setup:** The training-room access panel is ready. Mimi brings the same portable certification rack from intake: five current field badges on hooks 01–05, and hook 06 empty. Bibi stands beside it as archive mentor. Your earlier files hold everything needed for the final check.
- **Objective:** COMPLETE the four final checks using answers already in your book.
...
4. Use the call sign already written on your single Field Detective ID.
...
- **Room Zero / meta signal:** {"code": "MEMBER-30", "label": "HOOK 06", "reaction": "There was space beside us. You have done the work to take it.", "reader_move": "Use the same call sign on the certificate. Book 1 closes before the next file arrives.", "speaker": "nini", "status": "DETECTIVE SIX CERTIFIED"}
