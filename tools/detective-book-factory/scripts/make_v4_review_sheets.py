#!/usr/bin/env python3
"""Build the owner-requested V4 review sheets from final full-page previews."""
...
def object_sheet(report_path,map_dir,out):
    """Crop one real V4 map cell for every distinct spatial object type."""
    ...
    draw.text((gap,44),"Cropped from the final V4 puzzle maps at actual occupied-cell bounds.",fill="black",font=font(16))
...
def main():
    ...
    finale=list(range(idx["30_brief"],idx["book2_scene"]+1))
    visuals=[idx["03_puzzle"],idx["08_puzzle"],idx["11_puzzle"],idx["16_artifact"],idx["24_puzzle"],idx["27_artifact"]]
    groups=[
        ("full_book","FULL BOOK PAGE-BY-PAGE",list(range(1,idx["page_count"]+1)),12,4),
        ("front_matter","COMPLETE FRONT MATTER",list(range(1,13)),8,4),
        ("happy_makers","HAPPY MAKERS / LARGE CHARACTER INTRO",[3,4,5],6,3),
        ("invitation","DETECTIVE SIX / INVITATION / ONE ID",[6,7],4,2),
        ("case_index","PREMIUM CASE INDEX",[10,11],4,2),
        ...
        ("certificate","COLLECTIBLE CERTIFICATE",[idx["certificate"]],1,1),
        ("book2_hook","BOOK 2 CINEMATIC HOOK",[idx["book2_scene"]],1,1),
    ]
    count=0
    ...
    print(f"PASS: {count} V4 review sheets")
